...
const registerUser = asyncHandler(async (req, res) => {
    ...
    sendMail({
        subject: "THS_X: Email Verification",
        email,
    ...
    const result = await followModel.aggregate([
        {
            $facet: {
                followingCount: [
                    { $match: { followerId: userDetails._id } },
                    { $count: "count" },
                ],
                followersCount: [
                    { $match: { followeeId: userDetails._id } },
                    { $count: "count" },
                ],
                isFollowing: [
                    { $match: { followerId: userFound._id, followeeId: userDetails._id } },
                    { $count: "count" },
                ],
            },
        },
        {
            $project: {
                followingCount: { $arrayElemAt: ["$followingCount.count", 0] },
                followersCount: { $arrayElemAt: ["$followersCount.count", 0] },
                isFollowing: { $gt: [{ $arrayElemAt: ["$isFollowing.count", 0] }, 0] },
            },
        },
    ]);
...
//TODO (DONE): implement the same like cover upload (TEST)
const uploadCoverImage = asyncHandler(async (req, res) => {
    let { userId } = req.payload;

    let userFound = await userModel
        .findById(userId)
        .select("_id coverImg coverLocalPath");

    if (!userFound) {
        throw new ApiError(401, "User not found");
    }

    // Get the new cover URL (static and local)
    let newCoverUrl = getStaticFilePath(req);
    let newCoverLocalPath = getLocalPath(req.file.filename, req.file.fieldname);

    // Delete the old cover image file from the server
    let oldCoverImg = userFound.coverLocalPath; // Save old cover image URL for deletion later

    // Update user's cover image in DB
    userFound.coverImg = newCoverUrl;
    userFound.coverLocalPath = newCoverLocalPath;
    await userFound.save(); // Save changes in the DB

    // Delete the old file directly
    if (oldCoverImg) {
        await fs.unlink(oldCoverImg);
    }

    res
        .status(200)
        .json(new ApiResponse(200, "Cover Image Upload Successful", newCoverUrl));
});
